...
$orderid = clean($_GET['uid']);

$addorder = new editorde();
$addorder -> edit($con, $orderid);
              ...
              <div class="col-md-12">
                  <div class="card card-primary">
                      ...
                      <form name="register" role="form" onsubmit="" action="" method="post">
                          <?php # Select staff order from database
                            $msg = "SELECT _staff_id_, _item_id_, _quantity_, _date_, _dep_id_ FROM _orders_ WHERE _oID_ = ?";
                            $stmnt = $con -> prepare($msg);
                            $stmnt -> bind_param("i", $orderid);
                            $stmnt -> execute();
                            $stmnt -> bind_result($staffid, $itemid, $userqty, $selecteddate, $departid);
                            $stmnt -> fetch();
                            $stmnt -> close();
                          ?>
                          <div class="card-body">
                              <div class="form-group">
                                ...
                                <select class="form-control selectpicker" id="staff_list" name="staff" data-live-search="true">
                                    <?php # select all staff and select staff from order
                                        $msg = "SELECT _sID_, _name_, _surname_ FROM _staff_ INNER JOIN _name_ ON _name_._nID_ = _staff_._name_id_ WHERE _visible_ = 1 ORDER BY _name_";
                                        $stmnt = $con -> prepare($msg);
                                        $stmnt -> execute();
                                        $stmnt -> bind_result($sid, $staffname, $staffsur);
                                        while($stmnt -> fetch()){
                                            if($staffid == $sid){
                                                print '<option value="'.$sid.'" selected>'.clean($staffname).' '.clean($staffsur).'</option>';
                                            }else{
                                                print '<option value="'.$sid.'">'.clean($staffname).' '.clean($staffsur).'</option>';
                                            }
                                        }?>
                                      <option value="Notonlist">Not on list</option>
                                  </select>
                              </div>
                              <!-- show staff fields if not on list -->
                              <div class="form-group" id = "div_name">
                                <label for="user_name">Name</label>
                                <input class="form-control" id="user_name" type="name" name="user_name" placeholder="Name"/>
                              </div>
                              ...
                              <div class="form-group">
                                <label for="Department">Department</label>
                                  <select class="form-control" id="Department" name="depart">
                                      <?php #Select departments and select department from order
                                      $msg = "SELECT * FROM _departments_ WHERE _visible_=1";
                                        ...
                                        while($stmnt->fetch()){
                                            if($departid == $did){
                                                print '<option value="'.$did.'" selected> '.clean($department).'  
                                                </option>';
                                            }else{
                                                print '<option value="'.$did.'"> '.clean($department).'  
                                                </option>';
                                           }
                                      }?>
                                      <option value="Notonlist">Not on list</option>
                              ...
                              <!-- show department fields if not on list -->
                              ...
                              <div class="form-group">
                                <label for="quantity">Quantity</label>
                                  <input class="form-control" id="quantity" type="number" name="quantity" value="<?php print clean($userqty); ?>"/ required>
                              </div>
                              <div class="form-group">
                                <label for="Item">Item</label>
                                  <select class="form-control selectpicker" id="Item" name="item" data-live-search="true">
                                      <?php # Select item from list and select item from order
                                        ...
                                        while($stmnt->fetch()){
                                            if($oiid == $itemid){
                                                print '<option value="'.$oiid.'" selected> '.clean($brand).' '.clean($model).' £'.clean($price).'</option>';
                                            }else{
                                                print '<option value="'.$oiid.'"> '.clean($brand).' '.clean($model).' £'.clean($price).'</option>';
                                            }
                                       }?>
                                  </select>
                              </div>
                              <div class="form-group">
                                <label for="OrderDate">Date</label>
                                  <input class="form-control" id="OrderDate" name="order_date" type="date" value="<?php print clean($selecteddate); ?>"/ required>
                              </div>
                          </div>
                          <div class="card-footer">
                              <button class="btn btn-primary" style="float: right;" type="submit" name="Edit_order">Edit</button>
