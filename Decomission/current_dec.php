...
// Marr items as complete and remove from detailed list
if(isset($_GET['uid']) && !empty($_SESSION['login']) && $_SESSION['isadmin'] == 1) #check user account and session
{
    $adminid=clean($_GET['uid']);
    $date = date("d/m/Y");
    $msg = "UPDATE _decom_quantity_ SET _past_present_ = 0 AND _decom_date_= ? WHERE _dcID_ = ?"; # update general list to set it as past
    $stmnt = $con->prepare($msg);
    $stmnt -> bind_param("si", $date, $adminid);
    if($stmnt -> execute()){
        $stmnt->close();
        $msg = "UPDATE _decomission_ SET _dec_visible_ = 0 WHERE _dec_qty_ = ?"; # update detailed items list and mark as invisible
        $stmnt = $con -> prepare($msg);
        $stmnt -> bind_param("i", $adminid);
        $stmnt -> execute();
        $stmnt -> close();
        echo "<script>alert('Decomission has been moved to past');</script>";
        $extra="current_dec.php";
                ...
                <div class="card-header">
                  <!-- Top column -->
                    <div class="row">
                        ...
                        <div class="col-sm-12 col-md-1">
                            <a href="decomission_item.php">
                                <button class = "btn btn-block btn-primary" type="button" id="add_user">All items</button>
                            </a>
                        </div>
                        <?php if($_SESSION['isadmin'] == 1){ ?>
                        <div class="col-sm-12 col-md-1">
                ...
                <!-- /.row (main row) -->
                    <div class="card-body">
                      <table id="UserTable" class="table table-bordered table-striped">
                          <?php # select general data about decomission items
                                $msg = "SELECT _dcID_, _type_name_, _reason_, _quantity_ FROM _decom_quantity_ INNER JOIN _types_ ON _types_._tID_=_decom_quantity_._decom_ID INNER JOIN _reason_ ON _reason_._rID_=_decom_quantity_._reason_id_ WHERE _past_present_=1 AND _quantity_ > 0";
                                $stmnt = $con->prepare($msg);
                                $stmnt -> execute();
                                $stmnt -> bind_result($dcid, $tname, $reason, $qty);
                            ?>
                        <thead>
                        <tr>
                          <th>Type</th>
                                  ...
                                  <div class="col-10">
                                      <div class="row">
                                          <div class="col-sm-3 col-md-3">
                                              <a onclick="myFunction(<?php print clean($dcid) ?>)"><button class = "btn btn-success btn-xs" title="Mark as complete"><i class="fa fa-check"></i></button></a>
                                          </div>
                                      </div>
                                  </div>
                                </td>
