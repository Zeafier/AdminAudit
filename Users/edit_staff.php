...
$name=""; $sname=""; $surname=""; $email=""; $title=""; $nid=""; $nid1=""; $sid=""; $visible="";

$updatetstaff = new updateuser();
$updatetstaff -> insertstaff($con);
              ...
              <div class="col-md-12">
                  <div class="card card-primary">
                      ...
                      <form name="register" role="form" onsubmit="" action="" method="post">
                          <?php #select staff member information and put them into input fields
                                $msg = "SELECT n1._name_ AS _first_name_, n2._name_ AS _second_name_, _surname_, _email_, _title_id_ FROM _staff_ as s1 INNER JOIN _name_ as n1 ON n1._nID_=s1._name_id_ INNER JOIN _name_ as n2 ON s1._s_name_id_ = n2._nID_ WHERE s1._sID_=?";
                                $stmnt = $con->prepare($msg);
                                $stmnt -> bind_param("i", $userid);
                                $stmnt -> execute();
                                $stmnt -> bind_result($firstname, $secondname, $surname, $email, $titleid);
                                $stmnt -> fetch();
                                $stmnt -> close();
                          ?>
                          <div class="card-body">
                              <div class="form-group">
                                ...
                                <input class="form-control" id="user_name" name="user_name" type="name" value="<?php print $firstname; ?>"/ required>
                              </div>
                              <div class="form-group">
                                <label for="user_sname">Second Name</label>
                                  <input class="form-control" id="user_sname" name="user_sname" type="text" value="<?php if($secondname=="empty"){ print "";}else{print clean($secondname);} ?>"/>
                              </div>
                              <div class="form-group">
                                <label for="user_surname">Surname</label>
                                  <input class="form-control" id="user_surname" name="user_surname" type="surname" value="<?php print clean($surname); ?>"/ required>
                              </div>
                              <div class="form-group">
                                <label for="user_email">Email</label>
                                  <input class="form-control" id="user_email" name="user_email" type="email" value="<?php print clean($email); ?>"/ required>
                              </div>
                              <div class="form-group" id="div_title">
                                <label for="Title">Choose title</label>
                                  <select class="form-control" id="Title" name="Title">
                                      <?php 
                                            $stmnt = $con -> prepare("Select * FROM _title_ ORDER BY _title_name_"); # Select title and according to staff information put selected value
                                            $stmnt -> execute();
                                            $stmnt -> bind_result($id, $title_name);
                                             while ($stmnt->fetch()) {
                                                 if($id == $titleid){
                                                     echo '<option value="'.$title_name.'" selected>'.$title_name.'</option>'; 
                                                 }else{
                          ...
                          <div class="card-footer">
                              <input class="btn btn-primary" style="float: right;" type="submit" name="Update_staff" value="Update">
                              <button class="btn btn-danger" style="float: right; margin-right: 10px;" onclick="window.location = 'http://'+window.location.hostname+'/Users/staff_users.php'; return false" name="cancel">Cancel</button>
                          </div>
                      </form>
                  </div>
              </div>
