...
$departid = clean($_GET['uid']);


if(isset($_POST['update']) && !empty($_SESSION['login']) && $_SESSION['isadmin'] == 1){
    ...
    if(empty($deptname)){
        ...
        $stmnt->close();
    }else if ($stmnt -> fetch() && $depvis != 0){
        $stmnt->close();
        echo "<script>alert('Department with this name already exists');</script>";
        
    }else{
        $stmnt->close();
        $msg="UPDATE _departments_ SET _deparment_name_ = ? WHERE _dID_ = ?";
        $stmnt = $con->prepare($msg);
        $stmnt -> bind_param("si", $deptname, $departid);
        if($stmnt->execute()){
            echo "<script>alert('Department ".$deptname." has been updated');</script>";
        }
              ...
              <div class="col-md-12">
                  <div class="card card-primary">
                      ...
                      <form name="register" role="form" onsubmit="" action="" method="post">
                          <?php 
                            $msg = "SELECT _deparment_name_ FROM _departments_ WHERE _dID_ = ?";
                            $stmnt = $con->prepare($msg);
                            $stmnt -> bind_param("i", $departid); 
                            $stmnt -> execute();
                            $stmnt -> bind_result($dname);
                            $stmnt -> fetch();
                            $stmnt -> close();
                          ?>
                          <div class="card-body">
                              <div class="form-group">
                                ...
                                <input class="form-control" id="user_name" type="name" value="<?php print $dname;?>" name="department" / required>
                              </div>
                          </div>
